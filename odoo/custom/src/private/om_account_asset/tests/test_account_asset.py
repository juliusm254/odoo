...
class TestAccountAsset(common.TransactionCase):
    ...
    def test_00_account_asset_asset(self):
        ...
        ids = self.env["account.asset.depreciation.line"].search(
            [
                (
                    "asset_id",
                    "=",
                    self.ref("account_asset.account_asset_asset_vehicles_test0"),
                )
            ]
        )
        for line in ids:
        ...
        account_asset_asset_office0 = self.browse_ref(
            "account_asset.account_asset_asset_office_test0"
        )
        ...
        self.assertEqual(
            account_asset_asset_office0.method_number,
            len(account_asset_asset_office0.depreciation_line_ids),
        )
        # I compute a asset on period.
        context = {
            "active_ids": [
                self.ref("account_asset.menu_asset_depreciation_confirmation_wizard")
            ],
            "active_id": self.ref(
                "account_asset.menu_asset_depreciation_confirmation_wizard"
            ),
            "type": "sale",
        }
        asset_compute_period_0 = self.env[
