...
class AccountMove(models.Model):
    ...
    asset_ids = fields.One2many(
        "account.asset.asset", "invoice_id", string="Assets", copy=False
    )
        ...
        for move in self:
            if any(asset_id.state != "draft" for asset_id in move.asset_ids):
                raise ValidationError(
                    _("You cannot reset to draft for an entry having a posted asset")
                )
            if move.asset_ids:
    ...
    def action_post(self):
        ...
        for inv in self:
            ...
            for mv_line in inv.invoice_line_ids.filtered(
                lambda line: line.move_id.move_type in ("in_invoice", "out_invoice")
            ):
    ...
    def default_get(self, fields):
        ...
        if self.env.context.get("create_bill") and not self.asset_category_id:
            if (
                ...
                self.product_id
                and self.product_id.product_tmpl_id.asset_category_id
        ...
        for rec in self:
            if rec.product_id:
                if rec.move_id.move_type == "out_invoice":
                    ...
                    rec.asset_category_id = (
                        rec.product_id.product_tmpl_id.asset_category_id.id
                    )
        return vals
