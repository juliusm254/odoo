...
class PropertyProperty(models.Model):
    ...
    property_status = fields.Selection(
        [("no_building", "No Building"), ("has_building", "Has Building")],
    ...
    stakeholder_ids = fields.One2many(
        "property.stakeholder", "property_id", string="Stakeholders", auto_join=True
    )
...
class PropertyStakeHolder(models.Model):
    ...
    property_state = fields.Selection(string="State", related="property_id.state")
    ...
    def _set_percentage(self):
        for rec in self:
            if rec.owner_numerator and rec.owner_denominator:
                rec.percentage = math.ceil(
                    (rec.owner_numerator / rec.owner_denominator) * 100
                )
            else:
                rec.percentage = 0
