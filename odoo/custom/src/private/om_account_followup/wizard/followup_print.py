...
class FollowupPrint(models.TransientModel):
    ...
    def _get_followup(self):
        if self.env.context.get("active_model", "ir.ui.menu") == "followup.followup":
    ...
    company_id = fields.Many2one(
        "res.company", readonly=True, related="followup_id.company_id"
    )
    ...
    test_print = fields.Boolean(
        "Test Print",
        help="Check if you want to print follow-ups without "
    ...
    def process_partners(self, partner_ids, data):
        ...
        nbprints = 0
        resulttext = " "
        for partner in self.env["followup.stat.by.partner"].browse(partner_ids):
        ...
        for item in manuals:
            resulttext = (
                resulttext + "<li>" + item + ":" + str(manuals[item]) + "\n </li>"
            )
        resulttext += "</p>"
        result = {}
        ...
        for id in to_update.keys():
            if to_update[id]["partner_id"] in partner_list:
                self.env["account.move.line"].browse([int(id)]).write(
                    {"followup_line_id": to_update[id]["level"], "followup_date": date}
                )

    def clear_manual_actions(self, partner_list):
        partner_list_ids = [
            partner.partner_id.id
            for partner in self.env["followup.stat.by.partner"].browse(partner_list)
        ]
    ...
    def _get_partners_followp(self):
        ...
        fups = {}
        fup_id = (
            "followup_id" in context and context["followup_id"] or data.followup_id.id
        )
