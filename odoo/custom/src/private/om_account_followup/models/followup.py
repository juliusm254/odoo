...
class FollowupLine(models.Model):
    ...
    def default_get(self, default_fields):
        ...
        if self.env.ref(
            "om_account_followup.email_template_om_account_followup_default"
        ):
    ...
    followup_id = fields.Many2one(
        "followup.followup", "Follow Ups", required=True, ondelete="cascade"
    )
...
""",
    ...
    send_email = fields.Boolean(
        "Send an Email", default=True, help="When processing, it will send an email"
    )
    send_letter = fields.Boolean(
        "Send a Letter", default=True, help="When processing, it will print a letter"
    )
