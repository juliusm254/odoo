...
class FollowupStatByPartner(models.Model):
    ...
    max_followup_id = fields.Many2one(
        "followup.line", "Max Follow Up Level", readonly=True, ondelete="cascade"
    )
    balance = fields.Float("Balance", readonly=True)
    company_id = fields.Many2one("res.company", "Company", readonly=True)
    invoice_partner_id = fields.Many2one(
        "res.partner", compute="_get_invoice_partner_id", string="Invoice Address"
    )
