...
class ResPartner(models.Model):
        ...
        for partner in self:
            ...
            for aml in amls:
                ...
                if (
                    ...
                    and aml_followup
                    and (not latest_days or latest_days < aml_followup.delay)
                ):
                    latest_days = aml_followup.delay
                    latest_level = aml_followup.id
                if (
                    (aml.company_id == company)
                    and aml.followup_date
                    and (not latest_date or latest_date < aml.followup_date)
                ):
    ...
    def do_partner_manual_action_dermanord(self, followup_line):
        ...
        action_date = self.payment_next_action_date or fields.Date.today()
    ...
    def do_partner_manual_action(self, partner_ids):
        for partner in self.browse(partner_ids):
            ...
            action_date = partner.payment_next_action_date or fields.Date.today()
    ...
    def do_partner_print(self, wizard_partner_ids, data):
        ...
        datas = {"ids": wizard_partner_ids, "model": "followup.followup", "form": data}
        return self.env.ref("om_account_followup.action_report_followup").report_action(
            self, data=datas
        )
            ...
            if partners_to_email:
                ...
                if partner.payment_next_action_date:
                    payment_action_date = min(
                        fields.Date.today(), partner.payment_next_action_date
                    )
                else:
    ...
    def write(self, vals):
        if vals.get("payment_responsible_id", False):
            for part in self:
                if part.payment_responsible_id != self.env["res.users"].browse(
                    vals["payment_responsible_id"]
                ):
                    # Find partner_id of user put as responsible
                    responsible_partner_id = (
                        self.env["res.users"]
                        .browse(vals["payment_responsible_id"])
                        .partner_id.id
                    )
                    part.message_post(
                        body=_(
    ...
    def do_button_print(self):
        ...
        if not self.env["account.move.line"].search(
            ...
            raise ValidationError(
                _(
                    "The partner does not have any accounting entries to "
