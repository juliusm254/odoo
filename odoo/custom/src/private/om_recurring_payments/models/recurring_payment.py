...
class RecurringPayment(models.Model):
    ...
    journal_state = fields.Selection(
        required=True, string="Generate Journal As", related="template_id.journal_state"
    )

    description = fields.Text("Description")
    line_ids = fields.One2many(
        "recurring.payment.line", "recurring_payment_id", string="Recurring Lines"
    )
    ...
    def action_draft(self):
        if self.line_ids.filtered(lambda t: t.state == "done"):
            raise ValidationError(
                _("You cannot Set to Draft as one of the line is already in done state")
            )
        else:
    ...
    def create(self, vals):
        if "company_id" in vals:
            ...
            ).next_by_code("recurring.payment") or _("New")
        else:
...
class RecurringPaymentLine(models.Model):
    ...
    recurring_payment_id = fields.Many2one(
        "recurring.payment", string="Recurring Payment"
    )
