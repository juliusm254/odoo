...
class PropertyBuilding(models.Model):
    ...
    features_ids = fields.One2many(
        "property.features", "building_id", string="Features"
    )
    ...
    parking_space = fields.Integer(
        string="Parking Space", help="Parking Space Available"
    )
    ...
    surface_office = fields.Float(
        string="Surface office", compute="_compute_all_surface", store=True
    )  # Sbir
    surface_living = fields.Float(
        string="Surface living", compute="_compute_all_surface", store=True
    )  #
    surface_bedroom = fields.Float(
        string="Surface bedroom", compute="_compute_all_surface", store=True
    )  #

    surface_meeting = fields.Float(
        string="Surface meeting", compute="_compute_all_surface", store=True
    )  # Ssed
    surface_lobby = fields.Float(
        string="Surface lobby", compute="_compute_all_surface", store=True
    )  # Shol
    surface_staircase = fields.Float(
        string="Surface staircase", compute="_compute_all_surface", store=True
    )  # Scs
    surface_kitchen = fields.Float(
        string="Surface kitchen", compute="_compute_all_surface", store=True
    )  # Sof
    surface_sanitary = fields.Float(
        string="Surface sanitary", compute="_compute_all_surface", store=True
    )  # Sgrs

    surface_laboratory = fields.Float(
        string="Surface laboratory", compute="_compute_all_surface", store=True
    )  # Slb
    surface_it_endowments = fields.Float(
        string="Surface IT endowments", compute="_compute_all_surface", store=True
    )  # Sit
    surface_garage = fields.Float(
        string="Surface garage", compute="_compute_all_surface", store=True
    )  # Sgar
    surface_warehouse = fields.Float(
        string="Surface warehouse", compute="_compute_all_surface", store=True
    )  # Smag
    surface_log_warehouse = fields.Float(
        string="Surface logistic warehouse", compute="_compute_all_surface", store=True
    )  # Slog
    surface_archive = fields.Float(
        string="Surface archive", compute="_compute_all_surface", store=True
    )  # Sarh
    surface_cloakroom = fields.Float(
        string="Surface cloakroom", compute="_compute_all_surface", store=True
    )  # Sves
    surface_premises = fields.Float(
        string="Surface premises", compute="_compute_all_surface", store=True
    )  # Steh
    surface_access = fields.Float(
        string="Surface access", compute="_compute_all_surface", store=True
    )  # Sacc
    ...
    maintenance_team_type = fields.Selection(
    ...
    def onchange_purpose_parent_id(self):
        ...
        if self.purpose_parent_id:
            return {
                "domain": {
                    "purpose_id": [("parent_id", "=", self.purpose_parent_id.id)]
                }
            }
        else:
