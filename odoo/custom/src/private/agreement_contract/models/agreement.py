...
class AgreementContractWizard(models.TransientModel):
    ...
    def _generate_price_list(self, agreement):
        return (
            ...
            .create(
                {
                    "name": _("Price list for {}").format(
                        agreement.name
                    ),  # TODO: Possibly add some other identification, so that we can find the correct one for a specific agreement.
                    "item_ids": [
                        (0, 0, self._calculate_price_list_row(year))
    ...
    def save_button(self):
        ...
        self.env["contract.contract"].browse(contract_id)
